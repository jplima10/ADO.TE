...
def processa_pedido_adocao(request, id_pedido):
    ...
    if status == "A":
        pedido.status = "AP"
        string = '''Olá, sua adoão foi aprovada com sucesso'''
    elif status == "R":
        string = '''Olá, infelizemnte sua adoão foi recusada'''
        pedido.status = "R"

    pedido.save()

    email = send_mail('Sua adoção foi processada', string, 'joaopedrofontinelelima99@gmail.com', [pedido.usuario.email,])
